...
video_label.grid(row=0, column=0, rowspan=10, padx=10, pady=10)  # Video feed label
...
gaze_label = tk.Label(gaze_frame, text="GAZE DETECTION", font=("Arial", 14, "bold"))
gaze_label.pack(anchor="w")

gaze_center_label = tk.Label(gaze_frame, text="Center: ", font=("Arial", 12))
gaze_center_label.pack(anchor="w")

gaze_status_label = tk.Label(gaze_frame, text="Status: ", font=("Arial", 12))
...
head_label = tk.Label(head_frame, text="HEAD DETECTION", font=("Arial", 14, "bold"))
head_label.pack(anchor="w")

pitch_label = tk.Label(head_frame, text="Pitch: ", font=("Arial", 12))
pitch_label.pack(anchor="w")

yaw_label = tk.Label(head_frame, text="Yaw: ", font=("Arial", 12))
yaw_label.pack(anchor="w")

roll_label = tk.Label(head_frame, text="Roll: ", font=("Arial", 12))
roll_label.pack(anchor="w")

head_status_label = tk.Label(head_frame, text="Status: ", font=("Arial", 12))
...
distraction_label = tk.Label(distraction_frame, text="Distraction counts within 3 min:", font=("Arial", 12))
...
distraction_flag_label = tk.Label(distraction_frame, text="", font=("Arial", 14, "bold"))
distraction_flag_label.pack(anchor="w")



# ========================= YAWN AND BLINK SECTION ========================= #
yawnandblink_label = tk.Label(head_frame, text="YAWN AND BLINK DETECTION", font=("Arial", 14, "bold"))
yawnandblink_label.pack(anchor="w")
# Create labels for the drowsiness detection variables
blinks_label = tk.Label(head_frame, text=f"Blinks: {yb.num_of_blinks_gui}", font=("Arial", 12))
blinks_label.pack(anchor="w")

microsleep_label = tk.Label(head_frame, text=f"Microsleep Duration: {yb.microsleep_duration_gui:.2f}s", font=("Arial", 12))
microsleep_label.pack(anchor="w")

yawns_label = tk.Label(head_frame, text=f"Yawns: {yb.num_of_yawns_gui}", font=("Arial", 12))
yawns_label.pack(anchor="w")

yawn_duration_label = tk.Label(head_frame, text=f"Yawn Duration: {yb.yawn_duration_gui:.2f}s", font=("Arial", 12))
yawn_duration_label.pack(anchor="w")

blinks_per_minute_label = tk.Label(head_frame, text=f"Blinks Per Minute: {yb.blinks_per_minute_gui}", font=("Arial", 12))
blinks_per_minute_label.pack(anchor="w")

yawns_per_minute_label = tk.Label(head_frame, text=f"Yawns Per Minute: {yb.yawns_per_minute_gui}", font=("Arial", 12))
yawns_per_minute_label.pack(anchor="w")

alert_label = tk.Label(head_frame, text="", font=("Arial", 12, "bold"), fg="red")
...
def update_info():
    ...
    yawn_duration_label(text=f"yawn duration: {yb.yawn_duration_gui} sec", fg="black")
    blinks_per_minute_label(text=f"blinks/min: {yb.blinks_per_min_gui} sec", fg="black")
    yawns_per_minute_label(text=f"yawns/min: {yb.yawns_per_min_gui} sec", fg="black")
    ...
    if round(yb.microsleep_duration, 2) > microsleep_threshold:
        fatigue_alert_text = "⚠️ Alert: Prolonged Microsleep Detected!"
        fatigue_alert_color = "red"

    elif round(yb.yawn_duration, 2) > yawning_threshold:
        fatigue_alert_text = "⚠️ Alert: Prolonged Yawn Detected!"
        fatigue_alert_color = "orange"

    elif yb.microsleep_duration > microsleep_threshold:
        fatigue_alert_text = "⚠️ Alert! Possible Fatigue!"
        fatigue_alert_color = "red"

    elif yb.blinks_per_minute_gui > 35 or yb.yawns_per_minute_gui > 5:
        fatigue_alert_text = "⚠️ Alert! Driver is Highly Fatigued!"
        fatigue_alert_color = "red"
...
# Main function to capture frames and process them
def main():
    global cap
